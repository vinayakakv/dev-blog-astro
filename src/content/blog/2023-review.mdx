...
- [**HTTP Basic
  ...
  (see, now it is authorization again - HTTP Standards 😁). Sender and Reciever
...
the keys 🔑. In our case, the sender implemented HMAC authentication and we got
...
code in our NodeJS application (Scala to WASM? 🧐), we decided to go with a
...
function (look, no fancy `Reflection` 😛), which we hashed to get the
...
- In `f(a,b,c)`, if `a ` was of type `string`, you can't pass `number` as the
  first argument to `invalidate` (expect an article on how we did this 🙂). While
...
Car()`?) – so TypeScript is our go-to choice. That comes with an additional
...
person building the application itself – [yeah, jack of all
...
mutations being passed into it to generate the types (being _named_ is an
...
I think the moral of the story is that being lazy (_read: thinking out of the
box_) helps to expand the horizon of possible solutions, which might be more
...
1. Not making too many assumptions about the system, but ensuring we do not
   over-engineer anything. _We can't live without assumptions about the world that
   we live in_
...
collects data (like analytics data, but not analytics data 😜) and sends it to
our server. In development, we had to inject the script ourselves into the
webpage – Enter Browser Extensions.
...
> ✨ While I was writing this article, I learnt that `fetch` with [`keepalive:
...
costly - it was like _using an axe to kill a fly_. We turned out to simple
...
experience with it as everyone has with other cloud providers – no cloud
...
omits lots of small details — our embrace of the functional programming world
